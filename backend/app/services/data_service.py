...
from app.database import list_markers, upsert_markers

sample_features: list[dict[str, Any]] = [
    {"id": "flight-1", "type": "flight", "name": "AAL 123", "lat": 40.7128, "lng": -74.0060, "altitude": 10800, "status": "Cruising", "source": "sample"},
    {"id": "flight-2", "type": "flight", "name": "BAW 777", "lat": 51.5074, "lng": -0.1278, "altitude": 10668, "status": "Approaching", "source": "sample"},
    {"id": "flight-3", "type": "flight", "name": "SWA 200", "lat": 34.0522, "lng": -118.2437, "altitude": 9753, "status": "Cruising", "source": "sample"},
    {"id": "satellite-1", "type": "satellite", "name": "ISS", "lat": 39.0, "lng": -97.0, "altitude": 420, "status": "Orbiting", "source": "sample"},
    {"id": "satellite-2", "type": "satellite", "name": "Starlink-56", "lat": 10.0, "lng": 20.0, "altitude": 550, "status": "Orbiting", "source": "sample"},
    {"id": "camera-1", "type": "camera", "name": "Downtown CCTV", "lat": 47.6062, "lng": -122.3321, "altitude": 0, "status": "Live", "source": "sample"},
    {"id": "camera-2", "type": "camera", "name": "Harbor Cam", "lat": 25.2048, "lng": 55.2708, "altitude": 0, "status": "Live", "source": "sample"},
    {"id": "ground-1", "type": "ground", "name": "Seattle Weather", "lat": 47.6062, "lng": -122.3321, "altitude": 0, "status": "Clear", "source": "sample"},
    {"id": "ground-2", "type": "ground", "name": "Air Quality Berlin", "lat": 52.5200, "lng": 13.4050, "altitude": 0, "status": "Moderate", "source": "sample"},
]
...
def _fallback_features(kind: str) -> list[dict[str, Any]]:
    persisted = list_markers(marker_type=kind, limit=20)
    if persisted:
        return persisted
...
def _build_flight_features() -> list[dict[str, Any]]:
    ...
    if not payload or not payload.get("states"):
        return _fallback_features("flight")
    ...
    return features or _fallback_features("flight")
...
def _build_satellite_features() -> list[dict[str, Any]]:
    ...
    return _fallback_features("satellite")
...
def _build_ground_features() -> list[dict[str, Any]]:
    ...
    return features or _fallback_features("ground")
...
def get_map_features(marker_type: str | None = None, source: str | None = None, limit: int = 200) -> list[dict[str, Any]]:
    features = get_live_map_features()
    if marker_type:
        features = [item for item in features if item.get("type") == marker_type]
    if source:
        features = [item for item in features if item.get("source") == source]
